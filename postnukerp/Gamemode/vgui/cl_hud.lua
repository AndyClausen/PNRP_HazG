...
function HUD_Default()
	...
	--Draws the thirst text and bar
	PNRP_HUD:PaintText( x + offset, y, stat.THIRST, font, HUD_col.text )	
 	PNRP_HUD:PaintBar( x + offset, y + txtHeight + bar.txt_space, bar_width, bar.height, HUD_col.thirst_bar, thirstPerc )
	
	--If Thirst is below 20
	if Thirst < 20 then
 		local thx
 		local thy
 		thx = ScrW( ) / 2 -20
 		thy = ScrH( ) - txtHeight - 60
 		th_font = "TargetID"
 		local th_text = { }
 		th_text.shadow = Color( 0, 0, 0, 200 )
 		th_text.text = Color( 255, 255, 255, 255 * math.abs(math.sin(CurTime()*1.2)) )
		
 		PNRP_HUD:PaintText( thx, thy, "You need to drink!", th_font, th_text )
 		HUD_col.thirst_bar.fill = Color( 136, 136, 255 * math.abs(math.sin(CurTime()*2)), 255 )
 		HUD_col.thirst_bar.shade = Color( 0, 0, 219 * math.abs(math.sin(CurTime()*2)), 255 )
 		--Plays thirst sound
 		if ThirstSoundSW == 1 then
 		...
 		HUD_col.thirst_bar.shade = Color( 136, 136, 255, 255 )
		HUD_col.thirst_bar.fill = Color( 0, 0, 219, 255 )
 	end
	--Restes the sound switch
 	if Thirst >= 21 then
 		ThirstSoundSW = 1
