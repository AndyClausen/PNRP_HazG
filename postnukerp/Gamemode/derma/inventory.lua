...
function PNRP.build_inv_List(ply, itemtype, parent_frame, PropertySheet, MyInventory)
	...
	local pnlList = vgui.Create("DPanelList", PropertySheet)
		...
		for itemname, item in pairs(PNRP.Items) do
			local isFood = ""
			if item.Type == "zero" or item.Type == "one" or item.Type == "two" or item.Type == "three" or item.Type == "four" or item.Type == "five" or item.Type == "grown" or item.Type == "liquid" then isFood = "food" end
			local isWeapon = ""
			if item.Type == "melee" or item.Type == "pistol" or item.Type == "explosives" or item.Type == "assault" or item.Type == "battle" or item.Type == "smg" or item.Type == "shotgun" or item.Type == "sniper" or item.Type == "lmg" or item.Type == "attachment" then isWeapon = "weapon" end
			if item.Type == tostring( itemtype ) or isFood == tostring( itemtype ) or isWeapon == tostring( itemtype ) or tostring( itemtype ) == "all" then
				for k, v in pairs( MyInventory ) do
					if v > 0 then
						if string.lower(k) == string.lower(itemname) then
							...
							if item.Type == "food" or item.Type == "zero" or item.Type == "one" or item.Type == "two" or item.Type == "three" or item.Type == "four" or item.Type == "five" or item.Type == "medical" or item.Type == "weapon" or item.Type == "ammo" then
