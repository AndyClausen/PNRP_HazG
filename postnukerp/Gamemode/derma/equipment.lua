...
function ConvertWepEnt( weaponID )
...
function GM.EquipmentWindow( )
		...
		local AmmoListView = vgui.Create( "DListView", eq_frame )
			...
			for itemname, item in pairs( PNRP.Items ) do
				if item.Type == "ammo" then
					local ammoType = item.Name
					local ammoID = item.ID
					if ammoID == "ammo_ar2" then
						ammoType = "ar2"
					end
					
					local ammoCount = ply:GetAmmoCount( ammoType )
					--print("(equipment.lua) ammoID: "..ammoID)
					--print("(equipment.lua) ammoCount: "..ammoCount)
					if ammoCount > 0 then
						AmmoListView:AddLine( ammoType, ammoCount, ammoID)
		...
		local ammoSlideLabel = vgui.Create("DLabel", eq_frame)		
			...
			--	if !wepCheck then
				...
				if string.lower(v:GetModel()) == "models/weapons/v_hands.mdl" and string.lower(v:GetClass()) ~= "weapon_radio" then
					...
					if PNRP.FindWepItem(v:GetClass()) and checkGren(ply, v) or string.lower(v:GetClass()) == "weapon_radio" then
						local myItem
						...
						--	pnlPanel:SetTall(pnlList:GetTall() - 20)
							...
							if myItem.Type == "weapon" then
								pnlPanel.Icon:SetModel(myItem.Icon)
							else
	...
	--				    	pnlPanel.sendToInv:SizeToContents() 
			...
			--Attachments
			//CLIENT sends uniqueID to SERVER file via net
			net.Start("GetAtts")
				--print("CLIENT net.Start")
				net.WriteString(tostring(ply:UniqueID()))
			net.SendToServer()
			
			//CLIENT receives the gold
			net.Receive("SendAtts", function()
				--print("CLIENT net.Received!")
				doAttachments(net.ReadTable())
			end )
			function doAttachments(plyAtts)
				--print("doAttachments launched...")
				--print("Printing plyAtts:")
				--PrintTable(plyAtts)
				for k, v in pairs(plyAtts) do
					--print("Cur att: "..v)
					if PNRP.FindWepItem(v) then
						local myItem = PNRP.FindWepItem(v)
						...
						pnlPanel.Icon.DoClick = function()
							if ( v ) then
								net.Start( "pnrp_dropAttFromEQ" )
									net.WriteEntity(ply)
									net.WriteString(myItem.ID)
								net.SendToServer()
								eq_frame:Close()
								RunConsoleCommand("pnrp_stripWep",myItem.ID)
							end
						end
		...
		local dropAmmoBtn = vgui.Create("DImageButton", eq_frame)
			...
			dropAmmoBtn.DoClick = function() 
				...
				if AmmoListView:GetSelectedLine() then
					...
					local ammoFTYPE = ammoATYPE
					local ammoID = AmmoListView:GetLine(AmmoListView:GetSelectedLine()):GetValue(3)
					...
					RunConsoleCommand("pnrp_dropAmmo",ammoATYPE, ammoAMT, ammoID)
					eq_frame:Close()
				end
			end
		...
		if foundCar then		
			...
			local ammoToCarBtn = vgui.Create("DImageButton", eq_frame)
				...
				ammoToCarBtn.DoClick = function() 
					if AmmoListView:GetSelectedLine() then
						local ammoTypeSel = AmmoListView:GetLine(AmmoListView:GetSelectedLine()):GetValue(3)
						local ammoID = ammoTypeSel
...
function GM.BackpackWindow()
	...
	--		funnyLbl:SizeToContents()	
		...
		local pkPanel = vgui.Create( "DPanel", pack_frame )
			...
			//Don't drop certain items
			local function validItem(itm)
				local banItms = {"weapon_rape"}
				for k, id in pairs(banItms) do
					if id == itm.ID then
						--print(itm.ID.." is ban'd!")
						return false
					end
				end
				--print("Passed: "..itm.ID)
			...
			for k, v in pairs(contents.inv) do
				...
				if item and validItem(item) then
					local pnlPanel = vgui.Create("DPanel", Scroller)
				...
				if PNRP.Items[v] then
					...
					if (item.AllHide == true and not (ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1)) and not override then
						...
						pnlPanel.Icon = vgui.Create("SpawnIcon", pnlPanel)
							...
							pnlPanel.Icon.DoClick = function()
									RunConsoleCommand("pnrp_buildItem", v, passStr)
									tShopIntFrame:Close()
							end	
