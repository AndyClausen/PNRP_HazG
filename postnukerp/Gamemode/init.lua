...
-- --categories include (under items)
-- function AddItemsDir(dir)
	-- --print("[Adding Items] From: "..dir)
	
	-- local files, directories = file.Find(PNRP_Path.."gamemode/"..dir.."/*", "LUA")
	
	-- for _, fdir in pairs(directories) do
		-- local dirCheck = file.IsDir(PNRP_Path.."gamemode/"..dir.."/"..fdir, "LUA")
		-- if fdir != ".svn" and dirCheck then --Dont add the SVN Folders
			-- --print("Do Sub: "..dir.."/"..fdir)
			-- AddItemsDir(dir.."/"..fdir)
		-- else
			-- include(tostring(dir.."/"..fdir))
			-- --print("[Adding Items] File: "..tostring(dir.."/"..fdir))
		-- end
	-- end
	
	-- for _, v in pairs(files) do
		-- include(tostring(dir.."/"..v))
		-- --print("[Adding Items] File: "..tostring(dir.."/"..v))
	-- end
-- end
...
//--------hud_pda start--------\\
util.AddNetworkString("playerFreeze")
util.AddNetworkString("playerUnfreeze")

function PlayerFreeze()
	local pl = net.ReadEntity()
	-- pl:ChatPrint("Freezing...")
	pl:Freeze(true)
end
net.Receive("playerFreeze",PlayerFreeze)

function PlayerUnfreeze()
	local pl = net.ReadEntity()
	-- pl:ChatPrint("Unfreezing...")
	pl:Freeze(false)
end
net.Receive("playerUnfreeze",PlayerUnfreeze)
//---------hud_pda end---------\\
...
function SQLiteTableCheck ()
	...
	if not sql.TableExists("profiles") then
		query = "CREATE TABLE profiles ( pid INTEGER PRIMARY KEY AUTOINCREMENT, steamid varchar(255), model varchar(255), nick varchar(255), lastlog varchar(255), class int, xp int, skills varchar(255), health int, armor int, endurance int, hunger int, thirst int, res varchar(255), weapons varchar(255), ammo varchar(255) )"
		result = querySQL(query)
	else
		query = "SELECT thirst FROM profiles"
		result = sql.Query(query)
		if sql.LastError( result ) != nil and result == false then
			query = "ALTER TABLE profiles ADD COLUMN thirst int"
			Msg(tostring(os.date()).." missing values inserted to profiles.\n")
			querySQL(query)
		end
...
function GM:PlayerInitialSpawn( ply ) --"When the player first joins the server and spawns" function
	...
	ply:GetTable().LastThirstUpdate = 0
	ply:GetTable().Endurance = 100
	ply:GetTable().Hunger = 100
	ply:GetTable().Thirst = 100
...
function GM:PlayerLoadout( ply ) --Weapon/ammo/item function
 ...
 ---------------------------------------------------   
	-- if ply:SteamID()=="STEAM_0:0:20321511" or 
	-- ply:SteamID()=="STEAM_0:1:40259972" or 
	-- ply:SteamID()=="STEAM_0:1:6848048" then 
		-- ply:Give("")
	-- end --SHIT CODE FOR SPECFIC USERS
	//Rewrote this 4 u Wiktor <3 Just uncomment the below stuff (ctrl+shift+k):
	-- local coolPlayers={"STEAM_0:0:20321511","STEAM_0:1:40259972","STEAM_0:1:6848048"}
	-- for steamid,v in pairs(coolPlayer) do
