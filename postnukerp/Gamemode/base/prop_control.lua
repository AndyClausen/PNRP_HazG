...
function GM:PlayerSpawnedProp( ply, model, ent )
	...
	if GetConVarNumber("pnrp_propPay") == 1 then
		...
		if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminNoCost") == 1 then 
...
function GM:PlayerSpawnVehicle( p, class, vehtbl )
	if not (p:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1) then
...
function GM:PlayerGiveSWEP( ply, class, info )
	if not (ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1) then
...
function PuntCheck(ply, ent)
	if not (ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminTouchAll") == 1) then
...
function isPlayerAndAdmin( ent )
	if ent:IsPlayer() then
		if ent:IsSuperAdmin() or ent:IsUserGroup("superadmin") then
			return true
		else
			return false
		end
...
function PickupCheck( ply, ent)
	...
	if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminTouchAll") == 1 and not isPlayerAndAdmin( ent ) then return true end
...
function PhysUnfreezeCheck ( ply, ent, physobj )
	--admin can do whatever they want
	if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminTouchAll") == 1 then return true end
...
function ToolCheck( ply, tr, toolmode )
	...
	if toolmode == "wire_expression" or toolmode == "wire_expression2" or toolmode == "wire_gate_expression" or toolmode == "wire_debugger" or toolmode == "wire_adv" then
		if GetConVarNumber("pnrp_exp2Level") == 0 then 
			return false 
		elseif GetConVarNumber("pnrp_exp2Level") == 1 and ply:IsUserGroup("superadmin") then
			return true
		elseif GetConVarNumber("pnrp_exp2Level") == 2 and (ply:IsUserGroup("superadmin") or ply:Team() == TEAM_ENGINEER) then
			return true
		elseif GetConVarNumber("pnrp_exp2Level") == 3 and (ply:IsUserGroup("superadmin") or ply:Team() == TEAM_ENGINEER or ply:Team() == TEAM_SCIENCE) then
			return true
	...
	if GetConVarNumber("pnrp_toolLevel") == 1 and not ply:IsUserGroup("superadmin") then
