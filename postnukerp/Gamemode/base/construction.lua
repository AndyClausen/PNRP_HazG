...
function PNRP.SpawnBulkCrate( )
	...
	for itemname, item in pairs(PNRP.Items) do
		if tostring( ItemID ) == itemname then
			local allowed = false
			local enough = false
			--Admin overide and shophide
			if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1 then allowed = true 
			elseif item.ShopHide == true then allowed = false ply:ChatPrint("You cannot craft black items!") return end
			--If Right Class
			if team.GetName(ply:Team()) == item.ClassSpawn or item.ClassSpawn == "All" or allowed == true then
				
				//Some cultivator shit, yo
				local toNum = {["zero"]=0,["one"]=1,["two"]=2,["three"]=3,["four"]=4,["five"]=5,["liquid"]=1}
				local skillLvl = ply:GetSkill("Cooking")
				local reqLvl = toNum[item.Type] or item.Type
				if reqLvl==item.Type or skillLvl>=reqLvl then
					...
					ply:ChatPrint("Your level is too low, bitch! Your level: "..skillLvl.." Required level: "..reqLvl.." Get farming!")
				end
			else
...
function GM.BuildItem( ply, command, arg )
	for itemname, item in pairs(PNRP.Items) do
		if tostring( arg[1] ) == itemname then
			...
			if item.ShopHide == false or (ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1) or arg[2] == "firearmsbench" then
				if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminCreateAll") == 1 then allowed = true
					...
					if reqLvl==item.Type or skillLvl>=reqLvl then
						...
						if enough == true then
							...
							if ply:IsUserGroup("superadmin") and GetConVarNumber("pnrp_adminNoCost") == 1 then
								...
								elseif type(toolcheck) == "table" then
									local plyInv = PNRP.Inventory( ply )
									--PrintTable(toolcheck)
									for itemid, amount in pairs(toolcheck) do
										print("itemid: "..itemid.." amount: "..amount)
										print("PNRP.Items id"..PNRP.Items[itemid].ID)
				...
				ply:ChatPrint("You cannot craft black items!")
			end
		else
...
concommand.Add( "pnrp_buildItem", GM.BuildItem, q, p)
