...
util.AddNetworkString( "SendAtts" )
util.AddNetworkString( "GetAtts" )
//SERVER translates uniqueID to a ply
local function translateUID(UID)
	--print("translateUID launched...")
	for _, v in ipairs( player.GetAll() ) do
		--print("UID: "..UID)
		--print("v:UniqueID : "..v:UniqueID())
        if UID == tonumber(v:UniqueID()) then
			--print("v found")
			return v
		else
			--print("different...")
...
//SERVER sends ply.FAS2Attachments = {}
local function sendAtts(ply)
	--print("SERVER net.Start")
	net.Start("SendAtts")
		net.WriteTable(ply.FAS2Attachments)
	net.Send(ply)
end

//SERVER gets net like a hook
net.Receive("GetAtts", function()
	--print("SERVER net.Receive")
	sendAtts( translateUID( tonumber(net.ReadString()) ) )
end )

function PNRP.chtTest( ply, cmd, text )
	
	-- for k, v in pairs(ply:GetWeapons()) do
		-- ply:ChatPrint("Weapon name: "..v:GetClass())
		-- ply:ChatPrint("Wiktor's IQ: "..v:GetPrimaryAmmoType())
	-- end
end
PNRP.ChatCmd( "/test", PNRP.chtTest )

--//Drop Resources
function PNRP.DropRes( ply, text )
	//Check if someone's calling the command
	cmd = string.Explode( " ", string.lower(text) )
	if cmd[1] ~= "/drop" then return end
	
	//Basic vars and functions
	local resEntNames = {["chem"]="msc_chemnug",["small"]="msc_smallnug",["scrap"]="msc_scrapnug",["chemical"]="msc_chemnug",["smallpart"]="msc_smallnug"}
	local resGiveNames = {["chem"]="Chemicals",["small"]="Small_Parts",["scrap"]="Scrap",["chemical"]="Chemicals",["smallpart"]="Small_Parts"}
	local resNames = {["chem"]="Chemical",["small"]="Small Part",["scrap"]="Scrap",["chemical"]="Chemical",["smallpart"]="Small Part"}
	
	local function validRes(res)
		local rTable = {"chem","small","scrap","chemical","smallpart"}
	...
	if table.Count(cmd) ~= 3 then
		ply:ChatPrint("Invalid drop command!\nUsage: /drop [resource] [amount]\nExample: /drop chem 15")
		return
	end
	if not validRes(cmd[2]) then
		ply:ChatPrint("Invalid resource! Use either \"chem\", \"scrap\" or \"small\".")
		return
	end
	if isnumber(cmd[3]) or tonumber(cmd[3]) < 1 or tonumber(cmd[3])%1 ~= 0 then
		ply:ChatPrint("Invalid number! It needs to be a whole number above 0.")
		return
	elseif ply:GetResource(resGiveNames[cmd[2]])<tonumber(cmd[3]) then
		ply:ChatPrint("You don't have enough "..resNames[cmd[2]].."s!")
		return
	end
	
	//Spawn resource
	local pos = ply:TraceFromEyes(200).HitPos+Vector(0,0,20)
	ply:DecResource( resGiveNames[cmd[2]], cmd[3] )
	local resEnt = ents.Create( resEntNames[cmd[2]] )
		resEnt:SetAmount(tonumber(cmd[3]))
		resEnt:SetPos(pos)
		resEnt:Spawn()
		resEnt:Activate()
	local plural = ""
	if tonumber(cmd[3]) > 1 then plural = "s" end
	ply:ChatPrint("Dropped "..cmd[3].." "..resNames[cmd[2]]..plural..".")
	
end
hook.Add("PlayerSay", "PNRPDropRes", PNRP.DropRes)
...
meta.Name = function(self)
	if IsValid(self) then
		if self.rpname != nil then
			--print("Using rpname (ply_fun)")	
			return self.rpname
		else--if self:SteamName()!= nil then
			--print("Using SteamName (ply_fun): "..self:SteamName())
			--SName = self:SteamName()
			--return SName
		--else
			--print("Using SteamID (ply_fun)")
...
function HelpMenu(ply, cmd, args)
	if (not args[1]) or args[1] == "" or args[1] == nil then
		ply:ChatPrint("What do you need help with?")
		ply:ChatPrint("looting/rules/jews/items/shopmenu/crafting/status")
		ply:ChatPrint("Example: /help looting")
		return
	end
	local text = args[1]
	-- Teaches players how to loot
	if text=="looting" then
		ply:ChatPrint("To acquire scrap/small parts and chemicals you have to find loot piles, they have a variety of models like cars apc's phone booths etc, to loot the pile just press E")
		return
	end
	-- Tells players the basic rule set
	if text=="rules" then
		ply:ChatPrint("1. don't exploit 2. don't crash the server 3. don't lag the server 4. don't aboose wire 5. don't aboose")
		return
	end
	-- Tell players why jews are evil
	if text=="jews" then
		ply:ChatPrint("Man, fucking jews, they did everything 9/11, pearl harbour, my bitch, columbine and that is why Hitler did nothing wrong.")
		return
	end
	-- Tells players about items
	if text=="items" then
		ply:ChatPrint("Items can be found in the inventory menu and equipment menu, to take an item out the inventory menu simply press on icon or select more items and drop, same thing with equipment menu")
		return
	end
	-- tells players how to use the shopemenu
	if text=="shopmenu" then
		ply:ChatPrint("The shop menu is kind of like a crafting menu,look at the recipe list and resources amount and simply click on the icon to craft the item.")
		return
	end
	-- Tells players about crafting/crafting tools
	if text=="crafting" then
		ply:ChatPrint("To craft shit use the shop menu, ohh your here for guns ammo and all this other shit, craft the required bench first. ")
		return
	end
	if text=="status" then
		ply:ChatPrint("/status displays your current PDA stats in chat.Players can drop resources via /drop scrap 5")
		return
	else
		ply:ChatPrint("Invalid command! Check /help for list of commands")
		return
	end
end
concommand.Add( "pnrp_chathelp", HelpMenu )
PNRP.ChatConCmd( "/help", "pnrp_chathelp" )

concommand.Add( "pnrp_getstatus", function( ply, cmd, args )
	ply:ChatPrint("Your current status:")
		ply:ChatPrint("Health, "..ply:Health())
		ply:ChatPrint("Endurance, "..ply.Endurance)
		ply:ChatPrint("Hunger, "..ply.Hunger)
		ply:ChatPrint("Thirst, "..ply.Thirst)
end )
PNRP.ChatConCmd( "/status", "pnrp_getstatus" )
...
function GM.SaveCharacter(ply,cmd,args)
	...
	query = "UPDATE profiles SET model='"..ply:GetModel().."', nick ="..SQLStr(ply:Nick())..", lastlog="..SQLStr(os.date())..", class="..tostring(ply:Team())..", xp="..tostring(ply:GetXP())..", skills='"..skillList.."', health="..tostring(ply:Health())..", armor="..tostring(ply:Armor())..", endurance="..tostring(ply.Endurance)..", hunger="..tostring(ply.Hunger)..", thirst="..tostring(ply.Thirst)
...
function GM.CreateCharacter( ply, plyModel, plyClass )
	...
	query = "INSERT INTO profiles VALUES ( NULL, '"..ply:SteamID().."', "..SQLStr(plyModel)..", "..SQLStr(ply:Name())..", "..SQLStr(os.date())..", "..tonumber(plyClass)..", 0, ' ', 150, 0, 100, 100, 100, '0,0,0', ' ', ' ' )"
	result = querySQL(query)
...
--	ErrorNoHalt( "Stored HP:  "..result[1]["health"].."  After Set:  "..tostring(ply:Health()).."\n")
	...
	ply.Thirst = 100 --tonumber(result[1]["thirst"])
	--print("Thirst: "..tonumber(result[1]["thirst"]))
...
function PlyAcptPending(ply, cmd, args)
	...
	if overide == "force" and ply:IsUserGroup("superadmin") then
		cid = args[4]
	end
...
function GM:DoPlayerDeath( ply, attacker, dmginfo )
	...
	--Drop Weapons
	for k, v in pairs(ply:GetWeapons()) do
		local wepID = v:GetClass()
		
		local wepCheck = PNRP.CheckDefWeps(v) and v != "weapon_real_cs_admin_weapon"
		if !wepCheck then
			local myItem
			print("v:GetClass(): "..v:GetClass())
			if PNRP.FindWepItem(wepID) then
				myItem = PNRP.FindWepItem(v:GetClass())
			end
			...
			if ammoDrop > 0 then
				--ply:PrintMessage(3,"READ THIS WIKTOR: "..v:GetPrimaryAmmoType())
			...
			-- --	ply:SendHint( "Dropped "..myItem.Name,5)
				...
				elseif myItem.ID == "wep_shapedcharge" then
					...
					ammoFType = myItem.ID
					print("GetPrimaryAmmoType(): "..v:GetPrimaryAmmoType())
					print("myAmmoType: "..myAmmoType)
	...
	--Drop Attachments
	for k, v in pairs(ply.FAS2Attachments) do
		local wepID = v
		
		local myItem
		if PNRP.FindWepItem(wepID) then
			myItem = PNRP.FindWepItem(v)
		end
		contents.inv[myItem.ID] = 1
	end
	
	--Drop ammo without weapon
	local amt
	for i = 0, 70 do
		amt = ply:GetAmmoCount(i) or 0
		
		if amt > 0 then
			contents.ammo[PNRP.ConvertAmmoType(i)] = amt
		end
	end
...
function PNRP.ConvertAmmoType(ammoType)
	...
	if ammoType == 1 then sendType = "item_ammo_ar2"          end -- Ammunition of the AR2/Pulse Rifle
	if ammoType == 2 then sendType = "alyxgun"                end -- (name in-game "5.7mm Ammo") NOT NEEDED
	if ammoType == 3 then sendType = "pistol"                 end -- Ammunition of the 9MM Pistol NOT NEEEDED
	if ammoType == 4 then sendType = "smg1"                   end -- Ammunition of the SMG/MP7 NOT NEEDED
	if ammoType == 5 then sendType = "357"                    end -- Ammunition of the .357 Magnum NOT NEEDED
	if ammoType == 6 then sendType = "xbowbolt"               end -- Ammunition of the Crossbow NOT NEEEDED 
	if ammoType == 7 then sendType = "buckshot"               end -- Ammunition of the Shotgun NO NEED
	if ammoType == 8 then sendType = "rpg_round"              end -- Ammunition of the RPG/Rocket Launcher NO NEED
	if ammoType == 9 then sendType = "smg1_grenade"           end -- Ammunition for the SMG/MP7 grenade launcher (secondary fire)
	if ammoType == 12 then sendType = "sniperround"           end -- NO NEED
	if ammoType == 22 then sendType = "sniperpenetratedround" end -- (name in-game ".45 Ammo") NO NEED
	...
	--if ammoType == 26 then sendType = "fas2_ammo_357sig"      end -- Pistol Ammo P226
	if ammoType == 32 then sendType = "fas2_ammo_44mag"       end -- Revolver
	if ammoType == 34 then sendType = "fas2_ammo_45acp"       end -- M11A1 M1911 UZI Glock 20
	if ammoType == 38 then sendType = "fas2_ammo_50ae"       end -- deagle
	if ammoType == 40 then sendType = "fas2_ammo_50bmg"       end -- IMI Deagle M82 Raging bull
	if ammoType == 44 then sendType = "fas2_ammo_12gauge"     end -- TOZ-34 M3 Super KS-23 RM-870
	if ammoType == 54 then sendType = "fas2_ammo_545x39"      end -- AK-12 AK-74
	if ammoType == 56 then sendType = "fas2_ammo_556x45"      end -- Famas M4A1 Galil SR-25 SG550 SG552 G36
	if ammoType == 58 then sendType = "fas2_ammo_762x39"      end -- RK-95 RPK AKM SKS
	if ammoType == 60 then sendType = "fas2_ammo_762x51"      end -- M14 M21 G3 
	if ammoType == 62 then sendType = "fas2_ammo_9x18"        end -- PP19 OTS-33
	if ammoType == 64 then sendType = "fas2_ammo_9x19"        end -- OTS-33 MP5A6 MP5 MP5K 
	--if ammoType ==  then sendType = "" end
	--if ammoType ==  then sendType = "" end
...
util.AddNetworkString( "pnrp_dropAttFromEQ" )
function PNRP.DropAtt()
...
net.Receive( "pnrp_dropAttFromEQ", PNRP.DropAtt )
...
function PNRP.StripWep (ply, command, args)
	...
	elseif string.find( wep, "fas2_att_" ) then
		ply:FAS2_RemoveAttachment( string.gsub( wep, "fas2_att_", "" ) )
	else
		--print("not found..."..wep)
		ply:StripWeapon(wep)
	end
