...
function PNRP.OpenShopMenu()
	local ply = net.ReadEntity()
	local tbl = PNRP.Inventory( ply )
	
	if not tbl then
		tbl = {}
		print("Missing inventory! (base/inventory.lua)")
	end
	
	net.Start("pnrp_OpenShop")
		net.WriteTable(tbl)
	net.Send(ply)
end
util.AddNetworkString( "pnrp_OpenShop" )
util.AddNetworkString( "pnrp_OpenShopToServer" )
net.Receive("pnrp_OpenShopToServer",PNRP.OpenShopMenu)
...
function PNRP.AddToInvFromEQ( )
	...
	if command == "pnrp_addtoinvfromceq-ammo" then
		local ammoCount = p:GetAmmoCount(PNRP.Items[theitem].Name)
	...
	elseif command == "pnrp_addtoinvfromceq-ammo" then
		--print("(inventory.lua) test2")
		local ammoAmt = PNRP.Items[theitem].Energy
		if theitem == "ammo_ar2" then
			--print("(inventory.lua) ammo_ar2")
			p:RemoveAmmo( ammoAmt, theitem )
			p:ConCommand("pnrp_eqipment")
		else
			p:RemoveAmmo( ammoAmt, PNRP.Items[theitem].Name )
			--print("(inventory.lua) ammoAmt: "..ammoAmt.." theitem(ID) "..PNRP.Items[theitem].Name)
			p:ConCommand("pnrp_eqipment")
		end
	end
