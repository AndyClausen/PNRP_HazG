...
function PNRP.SetEventVar( ply, event, varname, value, vartype )
	if IsValid(ply) and not ply:IsUserGroup("superadmin") then return end
	...
	timer.Simple(5, function()
		...
		if validSpawn and util.IsInWorld(point) then
			...
			if math.random(100) <= 50 then
				...
				if rand1 <= 33 then
					contents.inv["weapon_pnrp_pulserifle"] = 1
				elseif rand1 <= 66 then
					contents.inv["m9k_combinesniper"] = 1
				else
					contents.inv["weapon_pulsemg"] = 1
				end
			else 
				local rand1 =  math.random(100)
				if rand1 <= 1 then
					contents.inv["fas2_m82"] = 1
				elseif rand1 <= 41 then
					contents.inv["fas2_m4a1"] = 1
				elseif rand1 <= 51 then
					contents.inv["fas2_mp5sd6"] = 1
				elseif rand1 <= 61 then
					contents.inv["fas2_sg550"] = 1
				elseif rand1 <= 71 then
					contents.inv["fas2_famas"] = 1
				elseif rand1 <= 100 then
					contents.inv["fas2_m3s90"] = 1
				end
			end
		...
		for i = 1, PNRP.Events["Prewar Insertion"].vars["Amount"] do
			...
			if validSpawn and util.IsInWorld(point) then
				...
				if math.random(100) <= 20 then
					...
					local rand1 = math.random(100)
					if rand1 <= 33 then
						contents.ammo["fas2_ammo_556x45"] = 60
...
----------------------------
--Chinese/Russian drop start
--////////////////////////--


-- local EVENT = {}
-- EVENT.name = "SARAC Insertion"

-- EVENT.vars = {}
-- EVENT.vars["Active"] = true
-- EVENT.vars["Chance"] = 100
-- EVENT.vars["CheckTime"] = 100
-- EVENT.vars["Amount"] = 4

-- EVENT.funcs = {}
-- function pnrp_ev_SARAC_OnLoad()
	-- local LoadInfo = PNRP.Events["SARAC Insertion"].funcs["LoadMapInfo"]
	-- LoadInfo()
	
	-- timer.Simple( 10, function()
		
		-- timer.Create( "pnrp_ev_sarac_check", PNRP.Events["SARAC Insertion"].vars["CheckTime"], 0, function ()
			-- if not PNRP.Events["SARAC Insertion"].vars["Active"] then return end
			-- local rand = math.random(100)
			-- if rand <= PNRP.Events["SARAC Insertion"].vars["Chance"] then
				-- local SpawnDrop = PNRP.Events["SARAC Insertion"].funcs["Spawn"]
				-- SpawnDrop()
			-- end
		-- end)
	-- end)
-- end
-- hook.Add( "InitPostEntity", "pnrp_ev_SARAC_OnLoad", pnrp_ev_SARAC_OnLoad )

-- function pnrp_ev_SARAC_Spawn()
	-- local GM = GAMEMODE
	-- local spawnTbl = GM.spawnTbl
	...
	-- for k, v in pairs(player.GetAll()) do
		-- v:ChatPrint("Something is happening... You're hearing some weird battle cries in what sounds like... Russian and Chinese?")
	-- end
	...
	-- if #posNodes > 0 then
		...
		-- while mainRetries > 0 and (not spawned) do
			...
			-- if validSpawn and util.IsInWorld(point) then
				...
				-- local Drop = PNRP.Events["SARAC Insertion"].funcs["DropPod"]
				-- Drop(point)
	...
	-- end
-- end
-- EVENT.funcs["Spawn"] = pnrp_ev_SARAC_Spawn

-- function pnrp_ev_SARAC_DropPod( pos )
	-- if pos == nil then return end
	...
	-- timer.Simple(5, function()
		...
		-- if validSpawn and util.IsInWorld(point) then
			-- local equipment = {"weapon_smg1","weapon_ar2"}
			-- local nadeAmount = {2,1}
			-- local models = {"models/nexuselite/fallout3/chinese commando.mdl","models/ninja/chinese_m_npc.mdl"}
			
			-- local ent = ents.Create("npc_combine_s")
			-- ent:SetKeyValue( "additionalequipment", "weapon_smg1" )
			-- ent:SetKeyValue( "squadname", "Chinese_Unit_1")
			-- ent:SetKeyValue( "NumGrenades", "2" );
			-- ent:SetKeyValue( "tacticalvariant", "true" );
			-- ent:SetKeyValue( "spawnflags", tostring(bit.bor(8192, 256, 4)) );
			-- ent:SetModel( "models/nexuselite/fallout3/chinese commando.mdl" )
			...
			-- ent:Fire( "StartPatrolling", "", 0.5 )
			-- ent:Fire( "SetSquad", "Chinese_Unit_1", 0.5 )
			...
			-- contents.res.chems = 0
			-- if math.random(100) <= 50 then
				-- contents.inv["weapon_smg1"] = 1
			-- else 
				-- local rand1 =  math.random(100)
				-- if rand1 <= 20 then
					-- contents.inv["fas2_racsa_rpk"] = 1
				-- elseif rand1 <= 35 then
					-- contents.inv["fas2_racsa_sks"] = 1
				-- elseif rand1 <= 40 then
					-- contents.inv["fas2_saiga"] = 1
				-- elseif rand1 <= 75 then
					-- contents.inv["fas2_ak74"] = 1
				-- elseif rand1 <= 95 then
					-- contents.inv["fas2_ak47"] = 1
				-- elseif rand1 <= 96 then
					-- contents.inv["fas2_svd"] = 1
				-- elseif rand1 <= 100 then
					-- contents.inv["fas2_ots33"] = 1
				-- end
			-- end
		...
		-- for i = 1, PNRP.Events["SARAC Insertion"].vars["Amount"] do
			-- point = Vector(pos.x + math.random(-250,250),
				  -- pos.y + math.random(-250,250),
				  -- pos.z)
	...
	-- end)
-- end
-- EVENT.funcs["DropPod"] = pnrp_ev_SARAC_DropPod

-- function pnrp_ev_SARAC_SaveMapInfo()
	-- if !file.IsDir("PostNukeRP", "DATA") then file.CreateDir("PostNukeRP") end
	-- if !file.IsDir("PostNukeRP/events", "DATA") then file.CreateDir("PostNukeRP/events") end
	-- if !file.IsDir("PostNukeRP/events/SARAC", "DATA") then file.CreateDir("PostNukeRP/events/SARAC") end
	
	-- local saveTable = {}
	-- for k, v in pairs(PNRP.Events["SARAC Insertion"].vars) do
		-- saveTable[k] = v
	-- end
	
	-- file.Write("PostNukeRP/events/SARAC/"..game.GetMap()..".txt",util.TableToJSON(saveTable))
-- end
-- EVENT.funcs["SaveMapInfo"] = pnrp_ev_SARAC_SaveMapInfo

-- function pnrp_ev_SARAC_LoadMapInfo()
	-- if file.Exists("PostNukeRP/events/SARAC/"..game.GetMap()..".txt", "DATA") then
		-- tbl = util.JSONToTable(file.Read("PostNukeRP/events/SARAC/"..game.GetMap()..".txt", "DATA"))
		
		-- for k, v in pairs(tbl) do
			-- PNRP.Events["SARAC Insertion"].vars[k] = v
		-- end
	-- end
-- end
-- EVENT.funcs["LoadMapInfo"] = pnrp_ev_SARAC_LoadMapInfo

-- PNRP.RegisterEvent(EVENT)
--////////////////////////--
--Chinese/Russian drop end--
----------------------------
