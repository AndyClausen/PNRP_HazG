...
function RemoveFromBackpack()
	...
	local function isWeapon( wepType )
		local wepList = {["weapon"]=1,["pistol"]=1,["smg"]=1,["shotgun"]=1,["battle"]=1,["assault"]=1,["sniper"]=1,["lmg"]=1,["explosives"]=1,["melee"]=1,["attachment"]=1}
		if wepList[wepType] then return true else return false end
	end
	...
	elseif option == "singledrop" then
		...
		if not myClass then
			myClass = tostring(removalTbl.ammo[1])
		end
	...
	elseif option == "equipall" then
		for k, v in pairs(backEnt.contents.inv) do
			if isWeapon(PNRP.Items[k].Type) then
	...
	elseif option == "takeall" then
		...
		for k, v in pairs(backEnt.contents.ammo) do
			local sngWeight = PNRP.Items[k].Weight
			local ciWeight = math.floor(v/PNRP.Items[k].Energy)
			
			if weightCap > currentCarry + ciWeight then
				if v/PNRP.Items[k].Energy > 0 then
					PNRP.AddToInventory( ply, k, math.floor(v/PNRP.Items[k].Energy) )
				end
				
				ply:GiveAmmo(v%PNRP.Items[k].Energy, k)
				backEnt.contents.ammo[k] = nil
				currentCarry = currentCarry + ciWeight
			elseif weightCap > currentCarry + sngWeight then
				local weightLeft = weightCap - currentCarry
				...
				if v/PNRP.Items[k].Energy > 0 then
					PNRP.AddToInventory( ply, k, canHave )
				end
				ply:GiveAmmo(v - (canHave * PNRP.Items[k].Energy), k)
				backEnt.contents.ammo[k] = nil
