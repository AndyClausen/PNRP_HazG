...
util.AddNetworkString("loadbiogasgen_stream")
util.AddNetworkString("unloadbiogasgen_stream")
util.AddNetworkString("togglebiogasgen_stream")
util.AddNetworkString("repgasbiogen_stream")
...
function ENT:Use( activator, caller )
	if activator:KeyPressed( IN_USE ) then
		if self.Repairing then
			...
			local availFuel = actInv["fuel_Zombiefule"]
		
			net.Start("biogasgen_menu")
...
util.AddNetworkString("biogasgen_menu")
...
net.Receive( "togglebiogasgen_stream", ENT.TogglePowerNet )
...
net.Receive( "repbiogasgen_stream", ENT.Repair )
...
function ENT.AddFuel()
	...
	local avail = plyInv["fuel_Zombiefule"]
	...
	PNRP.TakeFromInventoryBulk(ply, "fuel_Zombiefule", amnt)
	
	ply:ChatPrint("You have added "..tostring(amnt).." wafers of fuel.")
end
net.Receive( "loadbiogasgen_stream", ENT.AddFuel )
...
function ENT.RemFuel()
	...
	ent.FuelLevel = ent.FuelLevel - amnt
	ply:AddToInventory("fuel_Zombiefule", amnt)
	
	ply:ChatPrint("You have removed "..tostring(amnt).." wafers of fuel.")
end
net.Receive( "unloadbiogasgen_stream", ENT.RemFuel )
